...
export default function StaffPage() {
    ...
    const fetchStaff = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            // Query staff table with position and user joins
            // Only show staff with a position assigned (distinguishes staff from regular users)
            const { data, error } = await supabase
                .from("staff")
                .select(`
                    *,
                    position:staff_positions(id, name, color),
